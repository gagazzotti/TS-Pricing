...
class TestMellinOneSidedTS(unittest.TestCase):
    """TBD"""
    ...
    def test_double_sided(self):
        ...
        for strike in strikes:
            for ttm in ttms:
                ...
                ts_p_pricer = OneSidedTemperedStablePricer(
                    ts_params["alpha_p"], ts_params["beta_p"], ts_params["lambda_p"])
        ...
        print(
            f"\n Mellin Double sided => Ok! (Log10Error-max: {np.round(np.max(log10_error), 3)})"
        )
